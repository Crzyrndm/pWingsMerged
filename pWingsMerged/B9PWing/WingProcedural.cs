...
namespace ProceduralWings
{
    public class WingProcedural : ProceduralWing
    {
        ...
        public virtual Vector2 GetLimitsFromType(Vector4 set)
        {
        ...
        public static Vector4 sharedBaseLengthLimits = new Vector4(0.125f, 16f, 0.04f, 8f);
        public static Vector2 sharedBaseThicknessLimits = new Vector2(0.04f, 1f);
        public static Vector4 sharedBaseWidthRootLimits = new Vector4(0.125f, 16f, 0.04f, 1.6f);
        public static Vector4 sharedBaseWidthTipLimits = new Vector4(0.0001f, 16f, 0.04f, 1.6f);
        public static Vector4 sharedBaseOffsetLimits = new Vector4(-8f, 8f, -2f, 2f);
        public static Vector4 sharedEdgeTypeLimits = new Vector4(1f, 4f, 1f, 3f);
        public static Vector4 sharedEdgeWidthLimits = new Vector4(0f, 1f, 0f, 1f);
        public static Vector2 sharedMaterialLimits = new Vector2(0f, 4f);
        public static Vector2 sharedColorLimits = new Vector2(0f, 1f);
        ...
        public override void inheritShape(ProceduralWing parent)
        {
            ...
            length = Clamp(length, sharedBaseLengthLimits.x, sharedBaseLengthLimits.y);
            tipWidth = Clamp(tipWidth, sharedBaseWidthTipLimits.x, sharedBaseWidthTipLimits.y);
            tipOffset = Clamp(tipOffset, sharedBaseOffsetLimits.x, sharedBaseOffsetLimits.y);
            tipThickness = Clamp(rootThickness + length / parent.length * (parent.tipThickness - parent.rootThickness), sharedBaseThicknessLimits.x, sharedBaseThicknessLimits.y);
        ...
        public virtual void inheritEdges(ProceduralWing parent)
        {
            ...
            sharedEdgeTypeLeading = wing.sharedEdgeTypeLeading;
            sharedEdgeWidthLeadingRoot = wing.sharedEdgeWidthLeadingTip;
            sharedEdgeWidthLeadingTip = Mathf.Clamp(sharedEdgeWidthLeadingRoot + ((wing.sharedEdgeWidthLeadingTip - wing.sharedEdgeWidthLeadingRoot) / wing.sharedBaseLength) * sharedBaseLength, sharedEdgeWidthLimits.x, sharedEdgeWidthLimits.y);

            sharedEdgeTypeTrailing = wing.sharedEdgeTypeTrailing;
            sharedEdgeWidthTrailingRoot = wing.sharedEdgeWidthTrailingTip;
            sharedEdgeWidthTrailingTip = Mathf.Clamp(sharedEdgeWidthTrailingRoot + ((wing.sharedEdgeWidthTrailingTip - wing.sharedEdgeWidthTrailingRoot) / wing.sharedBaseLength) * sharedBaseLength, sharedEdgeWidthLimits.x, sharedEdgeWidthLimits.y);
        }
        ...
        public virtual void OnWindow(int window)
        {
            if (uiEditMode)
            {
                ...
                {
                    DrawField(ref sharedEdgeTypeTrailing, sharedIncrementInt, sharedIncrementInt, GetLimitsFromType(sharedEdgeTypeLimits), "Shape", uiColorSliderEdgeT, 10, 2, false);
        ...
        public virtual float SetupFieldValue(float value, Vector2 limits, float defaultValue)
        {
        ...
        public virtual void DrawField(ref float field, float increment, float incrementLarge, Vector2 limits, string name, Vector4 hsbColor, int fieldID, int fieldType, bool allowFine = true)
        {
            bool changed = false;
        ...
        public virtual void DrawFieldGroupHeader(ref bool fieldGroupBoolStatic, string header)
        {
            GUILayout.BeginHorizontal();
            if (GUILayout.Button(header, ProceduralWingManager.uiStyleLabelHint))
            {
            ...
            if (fieldGroupBoolStatic) GUILayout.Label("|", ProceduralWingManager.uiStyleLabelHint, GUILayout.MaxWidth(15f));
            else GUILayout.Label("+", ProceduralWingManager.uiStyleLabelHint, GUILayout.MaxWidth(15f));
            GUILayout.EndHorizontal();
        }
        ...
        public virtual Vector2 GetVertexUV2(float selectedLayer)
        {
            if (selectedLayer == 0)
        ...
        public virtual Color ColorHSBToRGB(Vector4 hsbColor)
        {
            float r = hsbColor.z;
            float g = hsbColor.z;
            float b = hsbColor.z;
